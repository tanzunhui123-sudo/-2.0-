...
export default function App() {
  ...
  const handleInpaintSubmit = async (maskBase64: string, prompt: string, referenceImage?: string, materialImage?: string) => {
     ...
     try {
        ...
        if (materialImage) {
             ...
             // Add reference image as the 3rd part
  ...
  const handleSendMessage = async () => {
    ...
    const processTask = async (image: string | null, taskIndex: number) => {
        ...
        try {
            if (mode === 'architect') {
                // --- Architect Mode (No changes here usually) ---
                ...
                const response = await generateWithRetry(
                    'gemini-3-pro-preview', 
                    {
                        contents: { parts },
                        config: {
                            systemInstruction: `You are Lyra, a professional "Prompt Architect" and visual director.
...
- Ensure high fidelity to the input image.`
                ...
                const responseText = response?.text || "解析失败，未能生成内容。";
                addMessage({ 
                  id: baseId, 
                  role: 'assistant', 
                  type: 'text', 
                ...
                // 2. Scene Reference Image (The Style/Environment)
                if (currentSceneRef) {
                ...
                if (image && currentSceneRef) {
                    // Scenario: Product + Style Reference
      ...
      <div className="flex-1 flex flex-col relative h-full">
        ...
        <div className="bg-white/95 dark:bg-slate-950/95 backdrop-blur-xl border-t border-slate-200 dark:border-slate-800 transition-all shadow-lg z-30">
          ...
          <div className="max-w-5xl mx-auto p-4 md:p-6 md:pb-6">
            ...
            <div className="flex items-end gap-3 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-2xl p-2 shadow-inner focus-within:ring-2 focus-within:ring-indigo-500/20 focus-within:border-indigo-500 transition-all">
